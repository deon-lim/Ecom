...
type RestOf<T extends IOrder | NewOrder> = Omit<T, 'dateTime'> & {
...
export class OrderService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/orders', 'order');
  ...
  protected convertDateFromClient<T extends IOrder | NewOrder | PartialUpdateOrder>(order: T): RestOf<T> {
    return {
      ...order,
      dateTime: order.dateTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOrder: RestOrder): IOrder {
    return {
      ...restOrder,
      dateTime: restOrder.dateTime ? dayjs(restOrder.dateTime) : undefined,
    };
  }
