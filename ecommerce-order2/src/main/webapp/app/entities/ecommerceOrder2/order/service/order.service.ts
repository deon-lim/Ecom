...
type RestOf<T extends IOrder | NewOrder> = Omit<T, 'createdOn'> & {
...
export class OrderService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/orders', 'ecommerceorder2');
  ...
  protected convertDateFromClient<T extends IOrder | NewOrder | PartialUpdateOrder>(order: T): RestOf<T> {
    return {
      ...order,
      createdOn: order.createdOn?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOrder: RestOrder): IOrder {
    return {
      ...restOrder,
      createdOn: restOrder.createdOn ? dayjs(restOrder.createdOn) : undefined,
    };
  }
